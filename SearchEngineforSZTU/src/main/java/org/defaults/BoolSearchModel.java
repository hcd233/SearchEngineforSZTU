...
public class BoolSearchModel extends InverseIndex{ // 继承自倒排索引类，这个子类只有新建的算法还有重写的算法
    ...
    public BoolSearchModel() { //构造方法 输入txt所在的文件夹进行倒排索引，初始化栈
        super();
        OperStack = new Stack<>();
        NumStack = new Stack<>();
    }
    ...
    public ArrayList<String> BoolSearch(String[] strings) throws IOException { // 布尔搜索算法，输入是空格分割的表达式{ 如 A & ( B | C )}转换成字符数组
        NumStack.clear();
        OperStack.clear();
        if(strings.length==2){
            ExactSearch(strings[0]);
        }
    ...
    public void ExactSearch(String str) throws IOException { // 精确搜索，不需要改动
        ...
        assert bls != null;
        for(String s:bls){
            GetFileContent(s,strarr.get((strarr.size() -1)/2));
        }
    }
    public ArrayList<JsonInfo> FuzzySearch(String str) throws IOException {
        ...
        for(int p=0;p<strs.length;p++){
            strb.append("( ");
            ArrayList<String> strarr = FuzzyInpStr(strs[p]);
            String  sign = " | ";
            if(strarr.size()==strs[p].length()){
                sign = " & ";
            }
            for(int i=0;i<strarr.size();i++){
                strb.append(strarr.get(i));
                words.add(strarr.get(i));
                if(i!=strarr.size()-1)
                    strb.append(sign);
            }
            strb.append(" )");
        ...
        ArrayList<String> bls = BoolSearch(strb.toString().split(" "));
//        HashSet<String> hashSet = new HashSet<>(bls);
//        ArrayList<String> AloneFile = new ArrayList<>(hashSet);//去重
//        int count;
//        HashMap<String,Integer> FileFreqCount = new HashMap<>();
//        for(String file:AloneFile){
//            count = 0;
//            for(String raw:bls){
//                if(Objects.equals(file, raw)) count++;
//            }
//            FileFreqCount.put(file,count);
//        }
//        TreeMap<String,Integer> SortedFreqCount = new TreeMap<>(FileFreqCount);
//        System.out.println(SortedFreqCount);
        System.out.println(bls);
        ArrayList<JsonInfo> res = new ArrayList<>();
        if(bls == null) return null;
        for(String i:bls){
           res.add(GetFileContent(i,words));
        }
        ArrayList<JsonInfo> resu = new ArrayList<>(res.stream().sorted(Comparator.comparing(JsonInfo::getCount).reversed()).collect(Collectors.toList()));
        if(resu.size()<20)
            return resu;
        else
            return new ArrayList<>(resu.subList(0,20));
    }
